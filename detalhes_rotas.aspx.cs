...
public partial class detalhes_rotas : System.Web.UI.Page
{ 
    //campos filtro
    int itotdados = 0;
    string _scampanha = "";
    ...
    public void AplicaFiltro()
    {
        try
        {
            ...
            Rel_DetalhesRota(_sperfil, _susuario, _sdtini, _sdtfim, _statusrota);
            ...
            if (IsPostBack == false)
            {
                ...
                foreach (string item in Request.QueryString)
                {
                    ...
                    switch (item)
                    {
                        ...
                        case "_statusrota":
                            _statusrota = Request.QueryString[item].ToString();
                            break;
    ...
    public void Rel_DetalhesRota(string sperfil, string susuario, string dtini, string dtfim, string sstatus)
    {
        try
        {
            ...
            using (SqlConnection cn = ogat.abre_cn())
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    ssql = @"select	* from vw_Detalhes_rotas_MP a (nolock) where 1=1";
