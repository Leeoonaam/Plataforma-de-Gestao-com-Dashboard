...
public partial class monitorar_servicos_netsms2 : System.Web.UI.Page
{ 
    //var
        ...
        try
        {
            ...
            using (SqlConnection cn = ogat.abre_cn())
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    //monta select
                    ssql = "SELECT distinct top 1  Robo,* FROM GATROBO (NOLOCK) WHERE Robo LIKE 'GAT Link RPA - CANCELAMENTO - OUTROS CANAIS%' order by DataStatus desc";
        ...
        try
        {
            ...
            using (SqlConnection cn = ogat.abre_cn())
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    //monta select
                    ssql = @"SELECT COUNT(1) FROM GATFilaRPACancelamento (NOLOCK) WHERE idcodstatus_rpa_cancelamento = 4";
        ...
        try
        {
            ...
            using (SqlConnection cn = ogat.abre_cn())
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    //monta select
                    ssql = @"SELECT COUNT(1) FROM GATFilaRPACancelamento (NOLOCK) WHERE idcodstatus_rpa_cancelamento = 1";
        ...
        try
        {
            ...
            using (SqlConnection cn = ogat.abre_cn())
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    //monta select
                    ssql = @"SELECT COUNT(1) FROM GATFilaRPACancelamento (NOLOCK) WHERE idcodstatus_rpa_cancelamento = 2 ";
        ...
        try
        {
            ...
            using (SqlConnection cn = ogat.abre_cn())
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    //monta select
                    ssql = @"SELECT COUNT(1) FROM GATFilaRPACancelamento (NOLOCK) WHERE idcodstatus_rpa_cancelamento in (3)"; //Erro e indevido
        ...
        try
        {
            ...
            using (SqlConnection cn = ogat.abre_cn())
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    //monta select
                    ssql = @"SELECT COUNT(1) FROM GATFilaRPACancelamento (NOLOCK) WHERE idcodstatus_rpa_cancelamento in (6)"; //Indevido
            ...
            using (SqlConnection cn = obj.abre_cn())
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    ssql = @"select	isnull(avg(DATEDIFF(second,data_ini_process_rpa_cancelamento,data_fim_process_rpa_cancelamento) ) ,0) 'tempo'
                            from	GATFilaRPACancelamento (nolock)
        ...
        try
        {
            ...
            using (SqlConnection cn = obj.abre_cn())
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    cmd.Connection = cn;
                    cmd.CommandText = "update GATFilaRPACancelamento set idcodstatus_rpa_cancelamento=4,data_rpa_cancelamento=getdate(),data_ini_process_rpa_cancelamento=null,data_fim_process_rpa_cancelamento=null,obs_rpa_cancelamento='' where isnull(idcodstatus_rpa_cancelamento,0) = 3";
                    cmd.CommandType = CommandType.Text;
    ...
    protected void gdw_dados_operadores_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (e.Row.Cells[1].Text == "CONTRATO CANCELADO COM SUCESSO!")
                {
                    e.Row.Cells[5].Text = "";
                }
                else
            ...
            if (e.CommandName == "liberar")
            {
                ...
                using (SqlConnection cn = obj.abre_cn())
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        cmd.Connection = cn;
                        cmd.CommandText = "update GATFilaRPACancelamento set idcodstatus_rpa_cancelamento=4,data_rpa_cancelamento=getdate(),data_ini_process_rpa_cancelamento=null,data_fim_process_rpa_cancelamento=null,obs_rpa_cancelamento='' where isnull(idcodstatus_rpa_cancelamento,0) = 3 AND obs_rpa_cancelamento = '" + sid + "'";
                        cmd.CommandType = CommandType.Text;
